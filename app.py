...
def answer_question_with_web_search(query: str):
    ...
    if not urls:
        app.logger.info("No URLs found or fetched successfully from Serper.")
        return {"answer": "Sorry, I couldn't find enough relevant websites for your query.", "sources": []}
    ...
    if vectorstore is None:
         ...
         # urls contains the URLs we *tried* to process, even if docs was empty
         return {"answer": "Sorry, I couldn't process the web content to set up the knowledge base.", "sources": urls}

    # 4. Setup RAG Chain
    qa_chain = setup_rag_chain(vectorstore)
    if qa_chain is None:
        # setup_rag_chain already prints specific error about Ollama
        app.logger.error("Failed to setup RAG chain with LLM.")
    ...
    try:
        ...
        # The query passed here is the original user question
        result = qa_chain.invoke({"query": query}) # Use invoke() as per Langchain standards
        ...
        app.logger.info(f"Answer generated. Used {len(source_documents)} source chunks from {len(sources)} unique sources.")
        app.logger.info(f"Sources used: {sources}")
